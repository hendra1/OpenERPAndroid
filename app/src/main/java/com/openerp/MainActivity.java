...
import com.openerp.addons.crm.EntryCashFlowActivity;
...
/*		sys.add(new DrawerItem(key, r.getString(R.string.title_about_us), 0,
    ...
    public void showFormInputIn(View view)
    {
        Intent intent = new Intent(this, EntryCashFlowActivity.class);
        String typeText = "Kas Masuk";
        //String categoryText = "Income";
    ...
    public void showFormInputOut(View view)
    {
        Intent intent = new Intent(this, EntryCashFlowActivity.class);
        String typeText = "Kas Keluar";
        String categoryText = "Income";

        intent.putExtra("typeText", typeText);
        //intent.putExtra("categoryText", typeText);
    ...
    public void sync(View view)
    {
        ...
        String[] myStrings = { ""};
        String where = "status = ?";
        String[] whereArgs = new String[] { "not sync" };
        List<OEDataRow> lDataRow = db.select(where,whereArgs,null,null,"id desc");

        mCashFlowItems.addAll(lDataRow);
        //List<OEDataRow> lDataRow2 = db.select(null,null,null,null,"id desc");
        //Log.d("All data: ", lDataRow2.toString());
        Log.d("filter data:",lDataRow.toString());

        OEHelper oe = db.getOEInstance();

        OEValues values = new OEValues();

        for(OEDataRow r: lDataRow){

            Log.d("Category Id relation : ", r.getM2ORecord("category").browse().getString("id"));

            int id = r.getM2ORecord("category").browse().getInt("id");

            values.put("date",r.getString("date"));
            values.put("description", r.getString("description"));
            values.put("amount", r.getString("amount"));
            values.put("category", id);
            values.put("type", r.getString("type"));
            values.put("status", "sync");
            //values.put("id", r.getInt("id"));

            oe.createToServerOnly(values, r.getInt("id"));
            Toast toast = Toast.makeText(this,"Sinkronisasi data Berhasil!", Toast.LENGTH_LONG);
